...
use tls_secret_validator::{
    warnings_for_secret_from_map, warnings_for_secret_rust, x509_key_pair_rust,
};
...
fn test_empty_data() {
    ...
    assert!(
        ...
        "Error should mention empty data or certificate: {}",
        err_msg
    );
}
...
fn test_warnings_for_secret() {
    ...
    assert!(
        warnings.is_empty(),
        "Non-TLS secrets should not generate warnings"
    );
    ...
    assert_eq!(
        warnings.len(),
        2,
        "Missing both fields should generate 2 warnings"
    );
    assert!(
        warnings.iter().any(|w| w.contains("tls.crt"))
            && warnings.iter().any(|w| w.contains("tls.key")),
        "Warnings should mention missing fields: {:?}",
        warnings
    );
    ...
    assert_eq!(
        warnings.len(),
        1,
        "Missing tls.key should generate 1 warning"
    );
    assert!(
        warnings[0].contains("tls.key"),
        "Warning should mention missing tls.key: {}",
        warnings[0]
    );
}
